...
namespace GenericSort.Sorting
{
  /// <summary>
  /// Provides generic implementations of major comparison-based sorting algorithms.
  /// </summary>
    ...
    where T : notnull, IComparable<T>
  {
    ...
    /// <summary>
    /// Recursively divides and merges parts of the array.
    /// </summary>
  ...
  /// <summary>
  /// Provides non-comparison-based sorting algorithms (Bucket, Counting, and Radix Sort).
  /// </summary>
  public static class NonComparableSorting
  {
    #region bucket sort
    /// <summary>
    /// Sorts an array of floating point numbers in the range [0,1) using Bucket Sort.
    /// </summary>
      ...
      where TFloating : IFloatingPoint<TFloating>
    {
      ...
      for (int i = 0; i < n; i++)
      {
        ...
        if (id < 0 || id >= n)
        {
          throw new InvalidOperationException("Numbers in A must be in [0, 1), reading " + A[i]);
        }
    ...
    /// <summary>
    /// Performs counting sort on an array of integers.
    /// </summary>
    /// <typeparam name="T">An integer type.</typeparam>
    /// <param name="A">The input array.</param>
    /// <param name="k">The maximum key value in the array.</param>
    /// <returns>A mapping of original indices after sorting.</returns>
    ...
    /// <summary>
    /// Performs counting sort and outputs the sorted result in array B.
    /// </summary>
    ...
    /// </summary>
    /// <param name="A">The array to sort.</param>
    /// <param name="k">The number of digits to process.</param>
    /// <param name="r">The radix (base). Defaults to 10.</param>
    public static void RadixSort(int[] A, int k, int r = 10)
    {
      ...
      for (int i = 0; i < k; i++)
      {
        for (int j = 0; j < n; j++)
        {
          digits[j] = (A[j] / ((int)Math.Pow(r, i))) % r;
        }

        int[] sortingMap = CountingSort<int>(digits, r - 1);
